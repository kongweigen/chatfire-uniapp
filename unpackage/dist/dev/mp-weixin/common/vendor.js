...
const toDisplayString = (val) => {
  return isString$2(val) ? val : val == null ? "" : isArray$1(val) || isObject$2(val) && (val.toString === objectToString || !isFunction$1(val.toString)) ? JSON.stringify(val, replacer, 2) : String(val);
};
...
function vFor(source2, renderItem) {
  ...
  if (isArray$1(source2) || isString$2(source2)) {
    ...
    if (!Number.isInteger(source2)) {
      warn$3(`The v-for range expect an integer value but got ${source2}.`);
      return [];
    }
...
function deepMerge$1(targetOrigin = {}, source2 = {}) {
  let target = deepClone(targetOrigin);
  if (typeof target !== "object" || typeof source2 !== "object")
    return false;
  for (const prop in source2) {
    if (!source2.hasOwnProperty(prop))
      continue;
    if (prop in target) {
      if (source2[prop] == null) {
        target[prop] = source2[prop];
      } else if (typeof target[prop] !== "object") {
        target[prop] = source2[prop];
      } else if (typeof source2[prop] !== "object") {
        target[prop] = source2[prop];
      } else if (target[prop].concat && source2[prop].concat) {
        target[prop] = target[prop].concat(source2[prop]);
      } else {
        target[prop] = deepMerge$1(target[prop], source2[prop]);
      }
    } else {
      target[prop] = source2[prop];
    }
...
function shallowMerge(target, source2 = {}) {
  if (typeof target !== "object" || typeof source2 !== "object")
    return false;
  for (const prop in source2) {
    if (!source2.hasOwnProperty(prop))
    ...
    if (prop in target) {
      if (source2[prop] == null) {
        ...
        target[prop] = shallowMerge(target[prop], source2[prop]);
      }
    } else {
      target[prop] = source2[prop];
    }
...
function getIconsTree(data, names) {
  ...
  (names || Object.keys(icons2).concat(Object.keys(aliases))).forEach(resolve2);
  return resolved;
}
...
function iconToSVG(icon, customisations) {
  ...
  [fullIcon, fullCustomisations].forEach((props2) => {
...
function iterateBrowserStorage(key, callback) {
  ...
  const version2 = getStoredItem(func2, browserCacheVersionKey);
...
function storeInBrowserStorage(storage2, data) {
  ...
  function store(key) {
    ...
    let index2;
    if (set3.size) {
      set3.delete(index2 = Array.from(set3).shift());
    } else {
      index2 = getBrowserStorageItemsCount(func2);
      if (index2 >= browserStorageLimit || !setBrowserStorageItemsCount(func2, index2 + 1)) {
        return;
      }
    }
    const item = {
...
function loadNewIcons(storage2, icons2) {
  ...
  if (!storage2.iconsQueueFlag) {
    ...
    setTimeout(() => {
      ...
      let api;
      if (!icons22 || !(api = getAPIModule(provider))) {
        return;
      }
...
const render = (icon, props2) => {
  const customisations = mergeCustomisations(defaultExtendedIconCustomisations, props2);
  const componentProps = { ...svgDefaults };
  const mode = props2.mode || "svg";
  const style = {};
  const propsStyle = props2.style;
...
const Icon = /* @__PURE__ */ defineComponent({
  ...
  render() {
    ...
    const icon = this.iconMounted || props2.ssr ? this.getIcon(props2.icon, props2.onLoad) : null;
    if (!icon) {
...
const digitToBasic = function(digit, flag2) {
...
class TokenTreeEmitter extends TokenTree {
   ...
   * @param {string} name
   */
  __addSublanguage(emitter2, name) {
    const node = emitter2.root;
    if (name)
...
const HLJS = function(hljs) {
  ...
  function _highlight(languageName, codeToHighlight, ignoreIllegals, continuation) {
    ...
    function processSubLanguage() {
      ...
      emitter2.__addSublanguage(result2._emitter, result2.language);
    }
    ...
    function emitKeyword(keyword, scope) {
      if (keyword === "")
        return;
      emitter2.startScope(scope);
      emitter2.addText(keyword);
      emitter2.endScope();
    }
    ...
    function startNewMode(mode, match2) {
      if (mode.scope && typeof mode.scope === "string") {
        emitter2.openNode(language.classNameAliases[mode.scope] || mode.scope);
      }
      if (mode.beginScope) {
    ...
    function processContinuations() {
      ...
      list2.forEach((item) => emitter2.openNode(item));
    }
    let lastMatch = {};
    ...
    const continuations = {};
    const emitter2 = new options.__emitter(options);
    ...
    try {
      if (!language.__emitTokens) {
        ...
        language.__emitTokens(codeToHighlight, emitter2);
      }
      emitter2.finalize();
      result = emitter2.toHTML();
      return {
        language: languageName,
        value: result,
        relevance,
  ...
  function removePlugin(plugin2) {
    const index2 = plugins.indexOf(plugin2);
    if (index2 !== -1) {
