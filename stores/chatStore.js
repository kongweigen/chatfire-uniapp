...
export const useChatStore = defineStore({
	...
	state: () => {
		return {
			...
			currentChatModel: "glm-4-9b-chat", // kimi
		}
	},
	getters: {
		chat: (state) => {
	...
	actions: {
		...
		setSettings(data) {
			...
			uni.setStorageSync("chatbot-api-settings", JSON.stringify(data))
		},
		setLoading(loading) {
		...
		// chat 相关
		initChat() {
			const chatListJson = uni.getStorageSync("chatbot-chat-list") || "[]"
		...
		createChat() {
			...
			uni.setStorageSync("chatbot-chat-list", JSON.stringify(this.chatList))
			this.setChat(item)
